...
class COCODataset(Dataset):
    ...
    def __init__(
        ...
        print('==========brightness_mean:', self.brightness_mean)
    ...
    def load_image(self, index):
        ...
        if self.img_mode in ['BGR', 'color', 'COLOR']:
            img = cv2.imread(img_file)
        elif self.img_mode == 'gray':
            img = cv2.imread(img_file, flags=cv2.IMREAD_GRAYSCALE)
            
            # 均值回归
            if self.brightness_mean is not None:
                delta_mean = self.brightness_mean - np.mean(img)
                img = img.astype(int)
                img += delta_mean
                img = np.clip(0, 255)
            ...
            raise NotImplemented(f"{self.img_mode} image mode is not implemented!")
