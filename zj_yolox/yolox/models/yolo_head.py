...
from yolox.models.varifocalloss import VarifocalLoss
from yolox.utils import bboxes_iou
from yolox.models.focal_loss import BinaryFocalLoss,QFocalLoss
from .losses import IOUloss
from .network_blocks import BaseConv, DWConv
from yolox.models.eqlv2 import EQLv2
...
class YOLOXHead(nn.Module):
    ...
    def __init__(
        ...
        self.eqlv2 = EQLv2(num_classes=self.num_classes)
        # self.qfocal_loss = QFocalLoss(nn.BCEWithLogitsLoss())
        self.qfocal_loss = QFocalLoss(nn.BCEWithLogitsLoss(pos_weight=torch.tensor([0.5])))
    ...
    def get_losses(
        ...
        # ====================================select obj loss =======================================
        if self.obj_loss == 'focal_loss':
            #loss_obj = self.focal_loss(obj_preds.view(-1, 1), obj_targets)
             loss_obj = ( self.focal_loss_v(obj_preds.sigmoid().view(-1, 1), obj_targets)).sum() / num_fg
            #  loss_obj = ( self.sigmoid_focal_loss(obj_preds.view(-1, 1), obj_targets)).sum() / num_fg
             
             
        elif self.obj_loss == 'varifocal_loss':
        ...
        loss_cls=0
        if self.cls_loss == 'qfocal_loss':
            
            loss_cls = self.qfocal_loss(cls_preds.view(-1, self.num_classes)[fg_masks],cls_targets) * 1000
        
        elif self.cls_loss == 'eqlv2':
            loss_cls = self.eqlv2(cls_preds.view(-1, self.num_classes)[fg_masks], cls_targets)
    ...
    def sigmoid_focal_loss(self, inputs: torch.Tensor, targets: torch.Tensor, alpha: float = 0.25, gamma: float = 2,
                           reduction: str = "sum", ):
    ...
    def dynamic_k_matching(self, cost, pair_wise_ious, gt_classes, num_gt,
        ...
        for gt_idx in range(num_gt):
            if (cost[gt_idx].shape[0] < dynamic_ks[gt_idx]):
                dynamic_ks[gt_idx] = cost[gt_idx].shape[0]
